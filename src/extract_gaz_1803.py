...
- An article typically begins with a line where the **name is in UPPERCASE**, followed by a **semicolon (;)**. Use this pattern to detect article starts.
- Extract the full **name** exactly as it appears, but exclude the semicolon if present.
- Extract the full unedited **text** of the article.
- **Do not summarize** - extract the full text.
- **Avoid indexes**: Any line that is just a 3-letter uppercase word (like `ABB`, `ABE`) and has **no semicolon** should be ignored. These are index entries and **not full articles**.
