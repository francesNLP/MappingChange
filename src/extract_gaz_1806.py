...
g_df_1806 = g_df[g_df['edition'] == '1806'].copy()
...
def merge_articles_with_pages(article_entries):
    merged = []
    article_map = {}  # map of base_name -> list of entries

    for entry in article_entries:
        base_name = format_article_name(entry["name"])
        text = entry["text"].strip()
        p_start = entry["page_start"]
        p_end = entry["page_finish"]

        #print("1. base_name %s, p_start %s, p_end %s" %(base_name, p_start, p_end))
        ...
        for existing in article_map[base_name]:
            same_page = p_start == existing["page_start"] and p_end == existing["page_finish"]
            overlapping = p_start <= existing["page_finish"] + 1 and p_end >= existing["page_start"] - 1

            # Merge if same name and same page and same text
            if same_page and text == existing["text"]:
                matched = True
                print("2. No merge: We are same name, page, text for %s" %base_name)
                break

            if same_page and text not in existing["text"]:
                matched = False
                print("3. No merge: We are same name, page, but different text for %s" %base_name)
                break

            # Merge if spanning adjacent pages and content is not the same (assume continuation)
            if overlapping and text not in existing["text"]:
                print("--- 3.Overlapping for base_name %s -- p_start:%s -- p_end:%s | existing[page_start]:%s -- existing[page_finish]:%s" %(base_name, p_start, p_end, existing["page_start"], existing["page_finish"]))
                existing["text"] += " " + text
                existing["page_start"] = min(existing["page_start"], p_start)
                existing["page_finish"] = max(existing["page_finish"], p_end)
                matched = True
                break

        if not matched:
            # Count how many times this name is used already
            #suffix = len(article_map[base_name])
            #new_name = f"{base_name}_{suffix}" if suffix > 0 else base_name
            new_name= base_name
            new_entry = {
                "name": new_name,
                "text": text,
            ...
            article_map[base_name].append(new_entry)
...
for start in range(0, num_pages, pages_per_chunk):
    ...
    g_df_1806_subset = g_df_1806_pages.iloc[start:end]
    marked_text = prepare_marked_text(g_df_1806_subset)
    raw_json_path = f"files/1806/main3/raw_article_entries_{start:04d}_{end-1:04d}.json"
    ...
    raw_filename = f"files/1806/main3/raw_extracted_articles_{start:04d}_{end-1:04d}.json"
    ...
    clean_filename = f"files/1806/main3/cleaned_articles_{start:04d}_{end-1:04d}.json"
